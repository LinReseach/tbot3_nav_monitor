...
class _SessionState:
    ...
    # First observed distance_remaining (straight-line "optimal" length proxy)
    ...
    last_feedback_xy: Optional[tuple] = None
...
class PerformanceMonitor(Node):
    ...
    def _finalize_session_and_publish(self, session: _SessionState) -> None:
        if session.finalized:
            self.get_logger().warn('finalize called twice for ' + session.goal_id[:8])
            return
        ...
        self.get_logger().info(
            f"[{alert_level}] goal={session.goal_id[:8]}... "
            f"t={exec_time:.1f}s acc={navigation_accuracy_m:.2f}m "
            f"rec={recovery_freq:.2f}/s eff={obstacle_eff:.2f} "
            f"bat={battery_units:.2f}"
        )
    ...
    def _on_action_status(self, msg: GoalStatusArray) -> None:
        # ROS2 Humble: GoalStatusArray uses .status_list, not .goals
        for goal_status in msg.status_list:
            ...
            if status_code == 2:  # EXECUTING (active)
                if goal_uuid_str not in self._sessions:
                    self._sessions[goal_uuid_str] = _SessionState(
                        goal_id=goal_uuid_str, start_time_ros=self._now_ros_sec()
                    )
            elif status_code in (4, 5, 6):  # SUCCEEDED / CANCELED / ABORTED
                session = self._sessions.get(goal_uuid_str)
                if session is None:
                    continue  # never tracked — old/replayed goal, skip it
